...
| MOVE-001 | Acceleration | Built | — |
| MOVE-002 | Steering | Built | — |
| MOVE-003 | Drift + mini-turbo | Built | — |
| MOVE-004 | Wall collision response | Built | — |
| CAM-001 | Chase camera | Built | — |
| TRACK-001 | Test track (Proving Grounds) | Built | — |
...
- **Spec:** Speed-sensitive turn rate — none parked, full through mid speed,
  mild fade at top speed; reverses when reversing. Front wheels + body yaw
  visualize it.
...
- **Spec:** Hold Shift+steer at speed → locked-direction drift with slip angle
  ...
  0.7s/1.4s mini-turbo boost. Verified: charge 1.19s → release → boost state
  0.5s → 28 m/s.
- **Evidence:** eval trace `["release+0.2",7.4,"boost",0.5]`; `slice-drift.png`
- **Critic Result:** —
- **Largest Gap:** drift auto-exits below 1.6 m/s then re-enters with reset
  charge during low-speed wall grind — charge-loss edge case on wall scrape.
...
- **Wave:** 1
- **Spec:** Kart clamps to road edge (no wall penetration); outward velocity
  reflected with restitution on impact + one-time 55% speed retention; sliding
  contact has light scrub only. Verified: kart slid along curbs, speed 9 km/h.
- **Evidence:** `slice-mid.png`, `slice-stuck.png`
- **Critic Result:** —
- **Largest Gap:** kart can wedge nose-first into wall needing counter-steer to
  recover — recovery forgiveness is a future unit.
...
- **Spec:** Exp-damped follow behind kart; look-ahead along heading; FOV
  60→74° with speed, +8° on boost. Frame-rate independent.
- **Evidence:** `slice-*.png` — consistent chase framing at 0 and 101 km/h
- **Critic Result:** —
- **Largest Gap:** —
...
- **Spec:** Closed Catmull-Rom circuit ~190×100 m (long straight, sweeper,
  S-curves, hairpin); 12 m road; red/white curbs; start stripe; centerline
  lookup for constraint + spawn.
- **Evidence:** `slice-drive2.png` — readable road ribbon + curbs
- **Critic Result:** —
- **Largest Gap:** flat, no elevation/jumps; placeholder visuals.
